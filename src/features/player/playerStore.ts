...
export const usePlayerStore = create<PlayerState>((set, get) => ({
  ...
  open: (video, queue) => {
    ...
    /**
     * Запись в историю — по факту открытия, а не по первому тику прогресса.
     *
     * Тики шлёт только нативный плеер. У встроенного плеера платформы
     * (WebView) их нет вообще, поэтому просмотренные так видео в историю
     * не попадали НИКОГДА — а у VK встроенный плеер единственный. Вкладка
     * «История» для VK была пуста, что бы пользователь ни смотрел.
     *
     * Именно `noteOpened`, а не `noteProgress(video, 0)`: позиция при
     * открытии ещё не известна, и запись нулём затирала бы сохранённую.
     */
    void useLibraryStore.getState().noteOpened(video);
  },
