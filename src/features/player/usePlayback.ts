 ...
 * открытия плеера, а не сохраняются вместе с карточкой.
 */
export function usePlayback(video: VideoSummary | null): PlaybackState {
  ...
  const provider = useMemo(
    () =>
      video && registry.has(video.providerId) ? registry.get(video.providerId) : null,
    [registry, video],
  );

  const preferEmbed = forceEmbed || !preferNativePlayer;
  const videoId = video?.id ?? null;
  const providerId = video?.providerId ?? null;

  // Смена видео — это новый сеанс: прошлый выбор «через веб-плеер» не должен
  // тянуться на следующий ролик, который, возможно, играется нативно.
  useEffect(() => {
    setForceEmbed(false);
  }, [videoId, providerId]);
  ...
  useEffect(() => {
    ...
    provider
      .resolvePlayback({ id: videoId, preferEmbed }, { signal: controller.signal })
      .then((resolved) => {
      ...
      .catch((cause: unknown) => {
        ...
        setSource(null);
        setError(ProviderError.from(cause, providerId ?? undefined).message);
        setStatus('error');
      });

    return () => controller.abort();
  }, [provider, videoId, providerId, preferEmbed, attempt]);
